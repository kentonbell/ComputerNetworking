...
def main():
    ...
    for i, line in enumerate(whois_lines):
        if "address" in line.lower():  # sometimes it's lowercase
            parts = line.split(':')
            if len(parts) < 2:
                continue  # malformed line
            address = parts[1].strip()

            if i + 2 < len(whois_lines):
                nextLine = whois_lines[i + 1]
                nextNextLine = whois_lines[i + 2]

                next_parts = nextLine.split(':')
                next_next_parts = nextNextLine.split(':')

                if len(next_parts) > 1:
                    cityStateZip = next_parts[1].strip()
                if len(next_next_parts) > 1:
                    thirdAddressLine = next_next_parts[1].strip()
            break
